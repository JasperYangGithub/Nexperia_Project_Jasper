...
class Encoder(nn.Module):
    ...
    def __init__(self, isize, nz, nc, ndf, ngpu, n_extra_layers=0, add_final_conv=True):
        ...
        if add_final_conv:
            ...
            main.add_module('final-{0}-{1}-conv'.format(cndf, 1),
                            nn.Conv2d(cndf, 1, 3, 1, 1, bias=False))
...
class Decoder(nn.Module):
    ...
    def __init__(self, isize, nz, nc, ngf, ngpu, n_extra_layers=0):
        ...
        main.add_module('initial-{0}-{1}-convt'.format(nz, cngf),
                        nn.ConvTranspose2d(nz, cngf, 3, 1, 1, bias=False))
...
class NetD(nn.Module):
    ...
    def __init__(self, opt):
        ...
        # self.classifier.add_module('classifier', nn.Conv2d(opt.nz, 1, 4, 1, 1, bias=False))
        # self.classifier.add_module('Sigmoid', nn.Sigmoid())
    ...
    def forward(self, x):
        ...
        # classifier = self.sigmoid_fun(last_layer)
        # classifier = classifier.view(-1, 1).squeeze(1)
