...
class BaseModel():
    ...
    def train_one_epoch(self):
        ...
        print("opt.lr = ", self.opt.lr)
        # print("opt.n_epochs = %d, opt.epoch = %d, opt.decay_epoch = %d" % (opt.n_epochs, opt.epoch, opt.decay_epoch))
        print("lr_scheduler_G.lr =", self.lr_scheduler_g.get_lr())
        print("lr_scheduler_G.lr =", self.lr_scheduler_d.get_lr())
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            # for i, data in enumerate(self.data.valid, 0):
                ...
                time_i = time.time()
                self.set_input(data)
                # self.input, self.gt = data
                # self.input, self.gt = self.input.to(self.device), self.gt.to(self.device)
                self.fake, latent_i, latent_o = self.netg(self.input)

                ## shuffle input
                # yy = self.input.clone()
                # print(yy)
                # idx = torch.randperm(yy.shape[0])
                # yy = yy[idx, :,:,:].view(yy.size())

            #-----------------Jasper add reconstruction error------------------------#
                si = self.input.size()
                rec = (self.input - self.fake).view(si[0], si[1] * si[2] * si[3])
                rec_error = torch.mean(torch.pow(rec, 2), dim=1)
                # -----------------Jasper add reconstruction error END------------------------#
                lat_error = torch.mean(torch.pow((latent_i-latent_o), 2), dim=1)
                #lat_error = lat_error.reshape(lat_error.size(0))
                #lat_error = lat_error.sum(2,1)

                # error = 6*rec_error + 4*lat_error
                error = torch.sum(lat_error,(1,2))
                time_o = time.time()
            ...
            auc, fpr_980, thr_980 = evaluate(self.gt_labels, self.an_scores, metric=self.opt.metric, saveto=os.path.join(self.opt.outf,self.opt.model,self.opt.dataset))
            performance = OrderedDict([('Avg Run Time (ms/batch)', self.times), (self.opt.metric, auc),
                                       ('fpr_980', fpr_980), ('thresh_980', thr_980)])
...
class Ganomaly(BaseModel):
    ...
    def __init__(self, opt, dataloader):
        ...
        self.real_label = torch.ones (size=(self.opt.batchsize, *(1,16,16)), dtype=torch.float32, device=self.device)
        self.fake_label = torch.zeros(size=(self.opt.batchsize, *(1,16,16)), dtype=torch.float32, device=self.device)
        ...
        if self.opt.isTrain:
            ...
            self.lr_scheduler_g = torch.optim.lr_scheduler.LambdaLR(
                self.optimizer_g, lr_lambda=LambdaLR(opt.niter, 0, 100).step)
            self.lr_scheduler_d = torch.optim.lr_scheduler.LambdaLR(
                self.optimizer_d, lr_lambda=LambdaLR(opt.niter, 0, 100).step)
    ##
    def forward_g(self):
    ...
    def backward_g(self):
        ...
        """
        self.err_g_adv = self.l_adv(self.netd(self.input)[0], self.netd(self.fake)[0])
    ...
    def backward_d(self):
        ...
        self.err_d_real = self.l_mse(self.pred_real, self.real_label)
        self.err_d_fake = self.l_mse(self.pred_fake, self.fake_label)
