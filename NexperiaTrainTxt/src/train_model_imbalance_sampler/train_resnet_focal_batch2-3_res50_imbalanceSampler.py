...
from tools.common_tools import ModelTrainer, show_confMat, plot_line, cal_auc, cal_focal_loss_alpha
...
if __name__ == "__main__":
    ...
    batch2_val_dir = os.path.join(BASE_DIR, "../..", "..", "..", "Data", "batch_2", "val")

    batch3_train_dir = os.path.join(BASE_DIR, "../..", "..", "..", "Data", "batch_3", "train")
    batch3_val_dir = os.path.join(BASE_DIR, "../..", "..", "..", "Data", "batch_3", "val")
    ...
    time_str = time_str + "_S_FL" + "_N-Pre" +"_Res50" + "_batch23"+ "_alphaNone"
    ...
    batch2_train_data = datasets.ImageFolder(batch2_train_dir, valid_transform)
    batch2_valid_data = datasets.ImageFolder(batch2_val_dir, valid_transform)
    batch3_train_data = datasets.ImageFolder(batch3_train_dir, valid_transform)
    batch3_valid_data = datasets.ImageFolder(batch3_val_dir, valid_transform)


    combined_data_train = torch.utils.data.ConcatDataset([batch2_train_data, batch3_train_data])
    combined_data_val = torch.utils.data.ConcatDataset([batch2_valid_data, batch3_valid_data])

    # alpha = cal_focal_loss_alpha(combined_data_train)
    # train_data = datasets.ImageFolder(train_dir, train_transform)
    # valid_data = datasets.ImageFolder(test_dir, valid_transform)

    # 构建DataLoder
    train_loader = DataLoader(dataset=combined_data_train, batch_size=BATCH_SIZE, shuffle=True, num_workers=4)
    valid_loader = DataLoader(dataset=combined_data_val, batch_size=BATCH_SIZE, num_workers=4)
    ...
    # alpha = [0.03571, 0.03571, 0.03571, 0.03571, 0.75, 0.03571, 0.03571, 0.03571]
...
# ============================ step 5/5 训练 ============================
    ...
    for epoch in range(start_epoch + 1, MAX_EPOCH):
        ...
        roc_auc_train, pr_auc_train, fpr_train = cal_auc(y_true_train, y_outputs_train, log_dir)
        roc_auc_valid, pr_auc_valid, fpr_valid = cal_auc(y_true_valid, y_outputs_valid, log_dir)
