...
from tools.common_tools import ModelTrainer, show_confMat, \
    plot_line, cal_auc_train, cal_focal_loss_alpha, cal_loss_eachClass,\
    dataset_info, SharpenImage, AddPepperNoise, set_seed, correct_label
...
if __name__ == "__main__":
    ...
    hard_sample_dir = os.path.join(BASE_DIR, "..", "..", "..", "..", 'Data', 'noisy_data','05-31_16-20_test_Res50_CE_NexCompareTrainSet_grayInput_transformSet_sharpPepperRand' )
    ...
    # name_test, labels_test = dataset_info(join(data_dir, 'Nex_trainingset_test.txt'))
    # ################# changle labels by using hardSample_trans4.txt #######################
    # name_correct, labels_correct = dataset_info(join(hard_sample_dir, 'hard_sample_trans4.txt'))
    # labels_train_correct = correct_label(name_train, labels_train, name_correct)
    # labels_val_correct = correct_label(name_val, labels_val, name_correct)
    # labels_test_correct = correct_label(name_test, labels_test, name_correct)

    now_time = datetime.now()
    time_str = datetime.strftime(now_time, '%m-%d_%H-%M')
    time_str = time_str + backbone + method + additional_info
