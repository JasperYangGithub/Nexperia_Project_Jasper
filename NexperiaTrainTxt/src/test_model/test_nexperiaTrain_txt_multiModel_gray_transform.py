...
from tools.common_tools import ModelTrainer, show_confMat, plot_line, cal_auc, \
    cal_focal_loss_alpha, cal_loss_eachClass, dataset_info, correct_label, cal_fpr_auc_mean_std, SharpenImage, Logger
...
if __name__ == "__main__":
    ...
    ###++++++++ for CE
    model_dir = '05-28_17-37_Res50_CE_grayInput_transformSet_SharpPepperRand'
    data_dir = os.path.join(BASE_DIR, "..", "..", "..", "..", "..", "share", "from_Nexperia_April2021", "Jan2021")
    backbone = "Res50"
    method = "CE"
    test_data_name = "Jan2021_down_img256_gray_transform_SharpPepperRand"

    name_test1, labels_test1 = dataset_info(join(data_dir, 'Jan2021_train_down.txt'))
    name_test2, labels_test2 = dataset_info(join(data_dir, 'Jan2021_val_down.txt'))
    name_test3, labels_test3 = dataset_info(join(data_dir, 'Jan2021_test_down.txt'))
    ...
    # labels = labels_test1_correct + labels_test2_correct + labels_test3_correct
    ...
    # ============================ step 2/5 模型 ============================
    ###### ResNet50 #######
    resnet_model = my_resnet.resnet50(num_classes=num_classes)
    ...
    ###### define fpr_980_list and auc_list to calculate mean and std #######
    fpr980_list = []
    auc_list = []
    ...
    for model_name in sorted(glob("*.pth")):
        ...
        fpr980_list.append(round(fpr_980_val*100, 2))
        auc_list.append(round(roc_auc_valid*100, 2))
    ...
    ###### calculate std and mean of AUC and FPR #######
    mean_list, std_list, mean_std_name = cal_fpr_auc_mean_std(fpr980_list)
    auc_mean_list, auc_std_list, auc_mean_std_name = cal_fpr_auc_mean_std(auc_list)
    for i in range(len(mean_list)):
        table.add_row([mean_std_name[i], ' ', ' ', ' ', str(mean_list[i])+'('+str(std_list[i])+')',
                       ' ', ' ', str(auc_mean_list[i])+'('+str(auc_std_list[i])+')'])
